...
int main() {
    ...
    while (1) {
		...
		while(rx != RXnumReady) rx = uart_receive();
		...
		pixel_step(x_coordinates[dandNum], y_coordinates[dandNum]);
	    ...
	    case 8:
		// Recalibration State
		// NO NEED FOR THIS STATE.. ADD INIT TO CALIB STATE
		uart_transmit(TX_Positioning);
		state = 5;
		break;
        }
